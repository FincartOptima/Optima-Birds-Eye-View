...
def _build_allocation_table(slide, asset_allocation, total_current_value, left, top, width):
    ...
    table.columns[2].width = int(width * 0.20)

    _set_cell(table.cell(0, 0), "Category", bold=True, size=11, color=_WHITE, fill=_NAVY)
    _set_cell(table.cell(0, 1), "Current Value", bold=True, size=11, color=_WHITE, fill=_NAVY, align=PP_ALIGN.RIGHT)
    _set_cell(table.cell(0, 2), "Weight", bold=True, size=11, color=_WHITE, fill=_NAVY, align=PP_ALIGN.RIGHT)

    for i, a in enumerate(asset_allocation, start=1):
        bg = _LIGHT_BLUE if i % 2 == 1 else _WHITE
        _set_cell(table.cell(i, 0), a["category"], size=10.5, fill=bg)
        _set_cell(table.cell(i, 1), _fmt_inr(a["value"]), size=10.5, fill=bg, align=PP_ALIGN.RIGHT)
        _set_cell(table.cell(i, 2), f"{a['pct']:.2f}%", size=10.5, fill=bg, align=PP_ALIGN.RIGHT)

    last = rows - 1
    _set_cell(table.cell(last, 0), "TOTAL", bold=True, size=10.5, fill=_CREAM)
    _set_cell(table.cell(last, 1), _fmt_inr(total_current_value), bold=True, size=10.5, fill=_CREAM, align=PP_ALIGN.RIGHT)
    _set_cell(table.cell(last, 2), "100.00%", bold=True, size=10.5, fill=_CREAM, align=PP_ALIGN.RIGHT)
    return graphic_frame


def _build_top_holdings_table(slide, top_holdings, left, top, width):
    ...
    _set_cell(table.cell(0, 0), "Rank", bold=True, size=11, color=_WHITE, fill=_NAVY)
    _set_cell(table.cell(0, 1), "Security", bold=True, size=11, color=_WHITE, fill=_NAVY)
    _set_cell(table.cell(0, 2), "% Assets", bold=True, size=11, color=_WHITE, fill=_NAVY, align=PP_ALIGN.RIGHT)

    for i, f in enumerate(top_holdings, start=1):
        bg = _LIGHT_BLUE if i % 2 == 1 else _WHITE
        _set_cell(table.cell(i, 0), str(i), size=10.5, fill=bg)
        _set_cell(table.cell(i, 1), f["scheme"], size=10.5, fill=bg)
        _set_cell(table.cell(i, 2), f"{f['pct']:.2f}%", size=10.5, fill=bg, align=PP_ALIGN.RIGHT)
    return graphic_frame


def _build_performance_table(slide, performance, left, top, width):
    ...
    _set_cell(table.cell(0, 0), "Portfolio / Benchmark", bold=True, size=11, color=_WHITE, fill=_DARK_NAVY)
    for i, label in enumerate(period_labels, start=1):
        _set_cell(table.cell(0, i), label, bold=True, size=11, color=_WHITE, fill=_DARK_NAVY, align=PP_ALIGN.RIGHT)

    _set_cell(table.cell(1, 0), "Overall Portfolio", bold=True, size=11, fill=_CREAM)
    for i, label in enumerate(period_labels, start=1):
        val = performance["portfolio"].get(label)
        _set_cell(table.cell(1, i), _fmt_pct(val), bold=True, size=11, fill=_CREAM, color=_return_color(val), align=PP_ALIGN.RIGHT)

    for r, bench in enumerate(filtered_benchmarks, start=2):
        bg = _LIGHT_BLUE if r % 2 == 0 else _WHITE
        _set_cell(table.cell(r, 0), bench["name"], size=11, fill=bg)
        for i, label in enumerate(period_labels, start=1):
            val = bench["returns"].get(label)
            _set_cell(table.cell(r, i), _fmt_pct(val), size=11, fill=bg, color=_return_color(val), align=PP_ALIGN.RIGHT)
    return graphic_frame


def _rebuild_slide_10(slide, data):
    """Remove the 3 labels + 4 example-data pictures, keep the background /
    header bar / title / logos, then add live tables in a cleaner 2-column
    layout (Broad Allocation + Top 5 Holdings stacked on the left, a roomier
    Performance table on the right — the old performance chart is dropped)."""
    ...
    label_h = Inches(0.32)
    label_gap = Inches(0.08)
    section_gap = Inches(0.22)

    alloc_label_top = Inches(0.95)
    alloc_table_top = alloc_label_top + label_h + label_gap
    _add_label(slide, "Broad Allocation", Inches(0.3), alloc_label_top, Inches(3), label_h)
    _build_allocation_table(
        slide, data["asset_allocation"], data["totals"]["current_value"],
        Inches(0.3), alloc_table_top, Inches(5.9),
    )

    n_alloc_rows = len(data["asset_allocation"]) + 2
    alloc_table_bottom = alloc_table_top + _ROW_H * n_alloc_rows
    top5_label_top = alloc_table_bottom + section_gap
    top5_table_top = top5_label_top + label_h + label_gap
    _add_label(slide, "Top 5 Holdings", Inches(0.3), top5_label_top, Inches(3), label_h)
    _build_top_holdings_table(
        slide, data["top_holdings"],
        Inches(0.3), top5_table_top, Inches(5.9),
    )

    inception = data.get("inception_date", "")
    _add_label(slide, f"Performance (Inception Date: {inception})", Inches(6.6), Inches(0.95), Inches(6.4), Inches(0.4))
    _build_performance_table(slide, data["performance"], Inches(6.6), Inches(1.7), Inches(6.4))
